...
| vercel-ship | v2.0 | Pre-deploy validation for Next.js + Vercel + Docker/k8s + concrete MCP deployment workflows. Serverless wall-clock-budget (the "network error" timeout), auth-gated internal-route 401 trap, vitest `@/`-alias gotcha. New: Hobby-plan cron/function limits (deploy-blocker), and the silent GitHub→Vercel auto-deploy webhook stall + the dashboard-reconnect fix |
