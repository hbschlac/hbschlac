...
| job-fetch | v1.1 | **Skill moved to `hbschlac/career-skills` (`skills/job-fetch/`) on 2026-09-23.** The plugin (`plugins/job-fetch/`, served from this repo's `.claude-plugin/marketplace.json`) now carries only the UserPromptSubmit hook that auto-fires on job URLs; it tells Claude to use the `job-fetch` skill, which resolves by name when `career-skills` is attached (and on the laptop via `~/.claude/skills`). Keep the `hooks` block in `.claude/settings.json` — see "Plugin hooks do not fire on web" below. |
...
| recruiter-filter | v1.3 | **Moved to `hbschlac/career-skills` (`skills/recruiter-filter/`) on 2026-09-23** with the other career skills. Attach `career-skills` to use it. Screens a CV against a job: knockout gate, 100-point rubric, fixes ranked by points recovered, Gmail outcome scan. |
