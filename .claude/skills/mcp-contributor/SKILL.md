...
Changes protocol spec or schema semantics → SEP (section 5). Otherwise → direct PR (section 3 or 4).
...
Ask: "Does this change what goes on the wire between client and server?"
- Yes → SEP
- No, but it changes how SDKs expose the protocol → issue + PR in the SDK repo
...
## Step 4: SDK and non-spec repo PRs
...
Use the template at `docs/specification/draft/sep-template.md` (if it exists) or follow existing SEPs as examples.
...
**Escalation:** If the same anchor miss appears in 3+ consecutive reports, it's stale. Fix the source, don't keep triaging it.

---

## Changelog

- **2026-05-19 — Consolidated from 20 session branches**
  - Documented the anchor-mismatch bug explicitly with fix instructions
  - Added drift response protocol with escalation rule
  - Added complete repo map (was missing Inspector, Registry, ext-* repos)
  - Retitled to clarify scope (all non-spec repos, not just SDKs)
