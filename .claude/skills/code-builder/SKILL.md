...
- Not a git repo → **offer `git init`**; if declined, force **single** (worktrees require git)
...
**Token budget awareness:** Each parallel agent costs ~50-150K tokens depending on task complexity. Before spawning:
- Estimate LOC × 1K tokens per draft as a rough floor
- For tasks <20 LOC estimated: single-pass is almost always better (3 agents × 50K = 150K tokens for a 5-line fix is wasteful)
- For tasks >100 LOC estimated: parallel-3 is justified; parallel-5 only for genuine architectural ambiguity
- If the session has already run 3+ parallel invocations today, prefer single-pass for remaining tasks unless the user explicitly requests parallel
...
**Prerequisite:** working directory must be a git repo. Run `git rev-parse --git-dir`. If it fails, ask: "This isn't a git repo. Want me to `git init` so I can use parallel drafts, or proceed single-pass?" If user declines, downgrade to single and re-announce.
...
5. **Deployment check.** If the project deploys to Vercel/Netlify/etc.:
   - Run `npm run build` (or equivalent) and verify zero errors
   - Check for new `process.env.X` references — each needs a corresponding Vercel env var or the deploy will silently fail
   - If the project has both `api/index.js` and `src/app.js` (dual entry), verify both are consistent
   - Watch for import paths that work locally but fail on Vercel (case sensitivity on Linux, missing file extensions)
...
Ensure the directory exists: `mkdir -p ~/.claude/skills/code-builder/runs`
...
If `Last synced:` date is more than 14 days ago, print before the activation banner:

> code-builder: learnings last synced {N} days ago. Patterns may be outdated. Run "code-builder sync" to refresh.

This prevents silently serving stale rules. The sync is the skill's core differentiator — if it's not running, the skill is degraded.

## Cross-skill coordination

**Conflicts:** If `debug-escalation` is already active for the current task (fix churn detected), do NOT spawn parallel drafts. Parallel drafts of a misunderstood bug produce N wrong answers.

**Precedence:** If `content-quality` would also activate (the task involves both code and user-facing text), run code-builder for the code changes and content-quality as a post-merge review pass on any user-facing strings in the diff.

**session-safety** always runs first (Step 0), regardless of which other skills are active.

## Banner throttle

If this skill has already announced 3+ times in the current session with "single pass", suppress subsequent single-pass banners. Only announce for parallel, debug-escalation, or when mode changes. This prevents banner fatigue during rapid single-pass tasks.

## Meta notes

- `isolation: "worktree"` fails when cwd is not a git repo. Offer `git init` before degrading.
- Token budget: each parallel agent costs ~50-150K tokens. Budget-aware judgment prevents waste.
- Cross-skill conflicts declared above. Skills activate independently but respect precedence.
