...
| Check | Action |
|---|---|
| Deploying to Vercel? | No `fs` in serverless. `trust proxy` if behind proxy. Verify BOTH dev entry AND serverless entry. Invoke vercel-ship if available. |
...
Log entry (append to `~/.claude/skills/code-builder/runs/{date}.jsonl`):
...
Last synced: 2026-05-29 (consolidated from 28 session branches + build-log analysis)
